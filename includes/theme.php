	...
	if(isset(til()->page)) {
		...
		$(document).ready(function(){ 
			...
			<?php if(is_array(@$arr['nav'])) {
				...
				if(!isset($arr['title'])) { $arr['title'] = 'نموذج اضافة موظف'; }

				if(isset($arr['title'])) {
					if(strlen($arr['title']) < 1) { $arr['title'] = 'المستقبل!'; }
					?>
						$('h3.page-title').html('<?php echo $arr["title"]; ?>');
						document.title = '<?php echo $arr["title"]; if($arr["title"] != 'Power ERP!') { echo ' | المستقبل!';} ?>';
					<?php
				}
	  ...
	  <ul class="pagination">
	  	<?php if($btn_after != '#'): ?>
	  		<li class="<?php if($btn_after == '#'): ?>disabled<?php endif; ?>">
	      		<a href="<?php echo $btn_after; ?>" aria-label="Previous">
	        		<span aria-hidden="true"><i class="fa fa-angle-double-left" aria-hidden="true"></i> <span class="hidden-xs">الصفحة الأولى</span></span>
	     		</a>
	    	</li>
	    <?php endif; ?>
	    ...
	    <?php if($btn_before != '#'): ?>
	    	<li class="<?php if($btn_before == '#'): ?>disabled<?php endif; ?>">
	        	...
	        	<span aria-hidden="true"><span class="hidden-xs">الصفحة الأخيرة</span> <i class="fa fa-angle-double-right" aria-hidden="true"></i></span>
	      		</a>
	    	</li>
	    <?php endif; ?>
	  </ul>
		...
		<div class="input-group">
			<div class="input-group-btn">
				<?php if( isset($arr['db-s-where']) ): ?>
				    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><span class="btn-text">عام</span> <span class="caret"></span></button>
				    <ul class="dropdown-menu dropdown-menu-left" role="menu">
				    	<li><a href="#" data-click="db-search" data-db-table-name="all">عام</a></li>
				    	<li role="separator" class="divider"></li>
				    	<li class="dropdown-header"><i class="fa fa-database"></i> بحث خاص</li>
		  ...
		  <input type="search" name="s" id="panel_s_<?php echo $arr['s_name']; ?>" class="form-control" placeholder="اكتب ماتريد البحث عنه هنا" value="<?php echo @$_GET['s']; ?>">
...
/**
 * get_theme_logs()
 * get_logs() fonksiyonundaki verileri "ultra" temasına gore suzer ve gosterir
 */
		...
		<table class="table table-hover table-condensed table-striped dataTable-logs">
			<thead>
				<tr>
					...
					<?php else: ?>
						<th class="none"></th>
						<th width="120" class="">التاريخ</th>
						<th width="200">المستخدم</th>
						<th>البيان</th>
					<?php endif; ?>
				</tr>
			</thead>
