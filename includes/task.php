		...
		if(!is_alert(__FUNCTION__)) {
			...
			if($q_insert = db()->query("INSERT INTO ".dbname('messages')." ".sql_insert_string($insert)." ")) {
				if(db()->insert_id) {
					...
					if($args['add_log'])	{ add_log(array('table_id'=>'messages:'.$insert_id, 'log_key'=>__FUNCTION__, 'log_text'=>'تم إرسال رسالة جديدة. '._b(get_user_info($rec_user->id, 'name').' '.get_user_info($rec_user->id, 'surname')))); }
					return $insert_id;
				} else { return false; }
	...
	if(!have_log(@$args['uniquetime'])) {
		if(!is_alert(__FUNCTION__, 'danger')) {
			...
			if($q_update = db()->query("UPDATE ".dbname("messages")." SET ".sql_update_string($update)." WHERE id='".$task_id."' ")) {
				if(db()->affected_rows) {
					if($args['add_alert']) { add_alert('تم تحديث معلومات المهمة.', 'success', __FUNCTION__); }
					return true;
				} else { return false; }
			} else { add_mysqli_error_log(__FUNCTION__); }
		} else { return false; }
	...
	if(!have_log(@$args['uniquetime'])) {
		if(!is_alert(__FUNCTION__)) {
			...
			if($q_insert = db()->query("INSERT INTO ".dbname('messages')." ".sql_insert_string($insert)." ")) {
				if(db()->insert_id) {
					$insert_id = db()->insert_id;
					if($args['add_alert']) 	{ add_alert('وأضاف رسالة الرد على المهمة.', 'success', __FUNCTION__); }
					if($args['add_log']) 	{ add_log(array('table_id'=>'messages:'.$task_id, 'log_key'=>__FUNCTION__, 'log_text'=>'تم إرسال رسالة الرد للمهمة. '._b('ID: '.$task->id))); }
