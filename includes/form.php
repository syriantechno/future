...
function set_form($id, $args=array()) {
	...
	if(!have_log()) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
		...
		@form_validation($set['account_code'], 'account_code', 'كود الحساب', 'min_length[3]|max_length[32]', __FUNCTION__);
		@form_validation($set['account_name'], 'account_name', 'اسم الحساب', 'min_length[3]|max_length[32]', __FUNCTION__);
		@form_validation($set['account_gsm'], 'account_gsm', 'الهاتف المحمول', 'gsm|min_length[5]|max_length[25]', __FUNCTION__);
		@form_validation($set['account_phone'], 'account_phone', 'الهاتف الثابت', 'min_length[5]|max_length[25]', __FUNCTION__);
		@form_validation($set['account_email'], 'account_email', 'البريد الالكتروني', 'email|min_length[3]|max_length[100]', __FUNCTION__);
		@form_validation($set['account_address'], 'account_address', 'العنوان', 'min_length[3]|max_length[500]', __FUNCTION__);
		@form_validation($set['account_district'], 'account_district', 'المقاطعة', 'min_length[3]|max_length[25]', __FUNCTION__);
		@form_validation($set['account_city'], 'account_city', 'المدينة', 'min_length[3]|max_length[25]', __FUNCTION__);
		@form_validation($set['account_country'], 'account_country', 'البلد', 'min_length[3]|max_length[30]', __FUNCTION__);
		@form_validation($set['account_tax_home'], 'account_tax_home', 'مكتب الضرائب', 'min_length[3]|max_length[30]', __FUNCTION__);
		@form_validation($set['account_tax_no'], 'account_tax_no', 'الرقم الضريبي', 'min_length[3]|max_length[25]', __FUNCTION__);
        

		if($operation == 'add') {
			if(!isset($set['date'])) { $set['date'] = date('Y-m-d H:i'); }
			@form_validation($set['date'], 'date', 'التاريخ', 'required|datetime', __FUNCTION__);
		...
		if(!is_alert(__FUNCTION__)) {
			...
			if(isset($set['new_account'])) {
				...
				$_account['Retardationnum'] 	= $_form['account_Retardationnum'];

				$_account['address'] 	= $_form_meta['account_address'];
				$_account['district'] 	= $_form_meta['account_district'];
				$_account['countrynumber'] 		= $_form['account_countrynumber'];
				...
				} else { add_alert('حدث الخطأ أثناء إنشاء بطاقة الحساب للنموذج.', 'danger', __FUNCTION__); return false; }
			}

			if(!empty($_form['account_code'])) {
				...
				} else { add_alert('"<b>'.$_form['account_code'].'</b>" لم يتم العثور على رمز الحساب.', 'danger', __FUNCTION__); return false; }
			}
			...
			if($operation == 'add' and !is_alert(__FUNCTION__)) {
				...
				if($q_insert = db()->query("INSERT INTO ".dbname('forms')." ".sql_insert_string($_form)." ")) {
					if(db()->affected_rows) {
						...
						add_alert('تم إنشاء النموذج.', 'success', __FUNCTION__);
						add_log(array('uniquetime'=>@$set['uniquetime'], 'table_id'=>'forms:'.$insert_id, 'log_key'=>__FUNCTION__, 'log_text'=>'تم إنشاء النموذج.'));
						$form = get_form($insert_id);

					} else {
						add_alert('خطأ غير معروف: لم يحدث أي إجراء في إنشاء النموذج.', 'warning', __FUNCTION__);
						return false;
					}
				} else { add_mysqli_error_log(__FUNCTION__); }
			} // if($operation == 'add')
			...
			if(!isset($form->id)) {
				if(!$form = get_form($id)) {
					add_alert('تم إيقاف النص البرمجي. لم يتم العثور على عنصر النموذج.', 'danger', __FUNCTION__);
					return false;
				}
			}
			...
			if($form and $operation == 'update' and !is_alert(__FUNCTION__, 'danger')) {
				...
				if($q_update = db()->query("UPDATE ".dbname('forms')." SET ".sql_update_string($_form)." WHERE id='".$form->id."' ")) {
					if(db()->affected_rows) {
						...
						add_alert('تم تحديث النموذج.', 'success', __FUNCTION__);
						add_log(array('uniquetime'=>@$set['uniquetime'], 'table_id'=>'forms:'.$id, 'log_key'=>__FUNCTION__, 'log_text'=>'تم تحديث النموذج.', 'meta'=>log_compare($old_form, $new_form)));
		...
		if($old_form = get_form($id)) {
			if($q_update = db()->query("UPDATE ".dbname('forms')." SET ".sql_update_string($update)." WHERE id='".$id."'")) {
				if(db()->affected_rows) {
					...
					if($args['add_alert']) { add_alert('تم تحديث النموذج.', 'success', __FUNCTION__); }
					if($args['add_log']) { add_log(array('uniquetime'=>@$args['uniquetime'], 'table_id'=>'forms:'.$id, 'log_key'=>__FUNCTION__, 'log_text'=>'تم تحديث النموذج.', 'meta'=>log_compare($old_form, $new_form))); }
					return true;
				} else { return false; }
			} else { add_mysqli_error_log(__FUNCTION__); }
		} else { return false; }
...
function get_form($id_or_query, $_til=true ) {
	...
	if($query = db()->query("SELECT * FROM ".dbname('forms')." ".sql_where_string($where)." ")) {
		if($query->num_rows) {
			...
			add_alert('لم يتم العثور على أي شكل للمعايير التي تبحث عنها. الآن <b>"'.$where['id'].'"</b> أنت تستفسر عن النموذج مع معرف.', 'warning', __FUNCTION__);
			return false;
		}
	} else { add_mysqli_error_log(__FUNCTION__); }
} // .get_form()
...
function get_forms($args=array()) {
	...
	if($query = db()->query($query_str)) { // sayfalama icin sorgu yapalim
		...
		if($return->num_rows > 0) {
			...
			add_alert('لا توجد بطاقات منتجات تطابق المعايير التي كنت تبحث عنها.', 'warning', __FUNCTION__);
			return false;
		}
	} else {
...
function calc_form($id) {
	...
	if($form = get_form($id)) {
		...
		add_console_log($id.' لم يتم العثور على أرقام المعرف.', __FUNCTION__);
		return false;
	}
} //.calc_form()
...
/**
 * get_in_out_label()
 * تجميد أسماء أنواع المدخلات / الإخراج
 */
...
function insert_form_item($args) {
	...
	if(!isset($insert['form_id'])) { add_alert('خطأ في المعلمة: لا يمكن أن يكون معرف النموذج فارغا.'); return false; }

	if(!have_log($args['uniquetime'])) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.

		if(!$form = get_form($insert['form_id'])) {
			add_alert(_b($insert['form_id']).' لم يتم العثور على رقم النموذج.', 'warning', __FUNCTION__); return false; }

		
		@form_validation($insert['date'], 'date', 'التاريخ', 'datetime', __FUNCTION__);	
		@form_validation($insert['item_code'], 'item_code', 'كود المادة', 'min_length[3]|max_length[32]', __FUNCTION__);	
		@form_validation($insert['quantity'], 'quantity', 'العدد', 'required|number|min_length[1]|max_length[10]', __FUNCTION__);
		@form_validation($insert['price'], 'price', 'سعر افرادي', 'money|min_length[1]|max_length[10]', __FUNCTION__);
		@form_validation($insert['vat'], 'vat', 'VAT', 'number|min_length[1]|max_length[10]', __FUNCTION__);
		print_alert();
		if(!is_alert(__FUNCTION__)) {
...
function add_form_item($form_id, $args=array(), $opt=array()) {
	...
	if(!have_log($args['uniquetime'])) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
		...
		if(!$form = get_form($form_id)) {
			add_alert('"<b>'.$form_id.']</b>" لم يتم العثور على رقم النموذج.', 'warning', __FUNCTION__);
			return false;
		}
		...
		if(!empty($args['item_code'])) {
			if(!$item = get_item(array('code'=>$args['item_code']))) {
				add_alert('"<b>'.$args['item_code'].'</b>" رمز المنتج غير موجود بين بطاقات المنتج.', 'danger', __FUNCTION__);
			} else {
		...
		if(!is_alert(__FUNCTION__)) {
			...
			if($q_insert = db()->query("INSERT INTO ".dbname('form_items')." ".sql_insert_string($args)." ")) {
				if(db()->affected_rows) {
					$insert_id = db()->insert_id;
					add_alert('تمت إضافة المنتج.', 'success', __FUNCTION__);
					add_log(array('uniquetime'=>@$args['uniquetime'], 'table_id'=>'forms:'.$args['form_id'], 'log_key'=>__FUNCTION__, 'log_text'=>'اضافة حركة المنتج. <small class="text-muted">'.$args['item_name'].'</small>'));
		...
		if($q_select = db()->query("SELECT * FROM ".dbname('form_items')." WHERE form_id='".$form_id."' AND id='".$id."' ")) {
			if($q_select->num_rows > 0) {
				...
				if($q_update = db()->query("UPDATE ".dbname('form_items')." SET status = 0 WHERE form_id = {$form_id} AND id= {$id} ")) {
                 ...
                 if($form_update = db()->query("UPDATE ".dbname('forms')." SET status = 0 WHERE id = {$form_id}")) {
                     $item = db()
                         ->query("SELECT * FROM ".dbname('items')." WHERE id ='{$form_item->item_id}'")
                         ->fetch_object();

                     $quantity = $item->quantity - $form_item->quantity;
                     @db()->query("UPDATE ".dbname('items')." SET quantity = {$quantity} WHERE id = {$form_item->item_id}");
                    if(db()->affected_rows) {
						add_alert('"<b>'.$form_item->item_name.'</b>" تم حذف حركة المنتج.', 'warning', __FUNCTION__);
						add_log(array('uniquetime'=>@$args['uniquetime'], 'table_id'=>'forms:'.$form_id, 'log_key'=>__FUNCTION__, 'log_text'=>'تم حذف حركة المنتج. <small class="text-muted">'.$form_item->item_name.'</small>'));
					} else {
						return false;
					}
                 }
				...
				add_alert('لم يتم العثور على حركة سينيسك الشكل.', 'danger', __FUNCTION__);
				return false;
			}
		...
		if($form = get_form($form_id)) {
			...
			if($q_select = db()->query("SELECT * FROM ".dbname('form_meta')." WHERE form_id='".$form_id."' AND meta_key='".$meta_key."' AND meta_value='".$meta_value."'  ")) {
				if($q_select->num_rows) {
					add_console_log($insert['name'].' شكل مفتاح معلومات إضافية، وأضاف سابقا.', __FUNCTION__);
					return false;
				} else {
...
function delete_form_meta($form_id, $args=array()) {
	...
	if($form = get_form($form_id)) {
		...
		if($q_delete = db()->query("DELETE FROM ".dbname('form_meta')." ".sql_where_string($where)." ")) {
			if(db()->affected_rows) { if($args['add_alert']) { add_alert('تم حذف النموذج معلومات إضافية.', 'warning', __FUNCTION__); } }
			return true;
		} else { add_mysqli_error_log(__FUNCTION__); }

	} else { if($args['add_alert']) { add_alert('لم يتم العثور على النموذج', 'warning', __FUNCTION__); } return false;  }
} //.delete_form_meta()
...
function add_form_status($args=array()) {
	if(!have_log(@$args['uniquetime'])) {
		...
		@form_validation($insert['taxonomy'], 'taxonomy', 'تصنيف (taxonomy)', 'required|min_length[3]|max_length[64]', __FUNCTION__);
		if(!is_alert(__FUNCTION__)) {
			...
			if($extra_id = add_extra($_insert)) {
				...
				if($args['add_alert']) { add_alert(_b($fs_cat_name).' إلى '._b($_insert['insert']['name']).' تمت إضافة حالة النموذج.', "success", __FUNCTION__); }
				if($args['add_log']) { add_log(array('uniquetime'=>@$args['uniquetime'], 'table_id'=>'extra:'.$extra_id, 'log_key'=>__FUNCTION__, 'log_text'=>_b($fs_cat_name).' إلى '._b($_insert['insert']['name']).' تمت إضافة حالة النموذج.')); }
				return $extra_id;
			} else { return false;}
		}
		...
		if($old_status = get_form_status($_where)) {
			...
			if($extra_id = update_extra($_where, $_args)) {
				if($extra = get_form_status($extra_id)) {
					...
					if($args['add_alert']) { add_alert(_b($fs_cat_name).' için '._b($extra->name).'</b>" تم تحديث حالة النموذج.', "success", __FUNCTION__); }
					if($args['add_log']) { add_log(array('uniquetime'=>@$args['uniquetime'], 'table_id'=>'extra:'.$extra->id, 'log_key'=>__FUNCTION__, 'log_text'=> _b($fs_cat_name).' için '._b($extra->name).' تم تحديث حالة النموذج.')); }
					return $extra->id;
				}
			} else { return false; }
		} else { if($args['add_alert']) { add_alert('لا يمكن العثور على عنصر حالة النموذج المراد تحديثه.', "warning", __FUNCTION__); } return false; }
	...
	if(delete_extra($_args)) {
		...
		if($new_status = get_form_status($new_status_id)) {
			if(db()->query("UPDATE ".dbname('forms')." SET status_id='".$new_status_id."' WHERE status_id='".$delete_status_id."' ")) {
 				if(db()->affected_rows) {
 					$fs_cat_name = til()->form_status[$delete_status->taxonomy]['name'];
					if($args['add_alert']) { add_alert(_b($fs_cat_name).' إلى '._b($delete_status->name).'</b>" جميع أشكال وضع النموذج '.$new_status->name.' تم استبداله.', "success", __FUNCTION__); }
					if($args['add_log']) { add_log(array('table_id'=>'extra:'.$extra->id, 'log_key'=>__FUNCTION__, 'log_text'=> _b($fs_cat_name).' إلى '._b($delete_status->name).'</b>" جميع أشكال وضع النموذج '.$new_status->name.' تم استبداله.')); }
 					return true;
 				} else { return false; }
			} else { add_mysqli_error_log(__FUNCTION__); }
		}
		...
		if($args['add_alert']) { add_alert(_b($fs_cat_name).' إلى '._b($delete_status->name).'</b>" تم حذف حالة النموذج.', "success", __FUNCTION__); }
		if($args['add_log']) { add_log(array('table_id'=>'extra:'.$delete_status->id, 'log_key'=>__FUNCTION__, 'log_text'=> _b($fs_cat_name).' إلى '._b($delete_status->name).' تم حذف حالة النموذج.')); }
		return true;
	} else { return false; }
}
...
function change_form_status($form_id, $args) {
	...
	if($form_status = get_form_status($where)) {
		...
		if(update_form($form_id, $_args)) {
			add_alert('حالة النموذج "<b>'.$form_status->name.'</b>" تم تغيير.', 'success', __FUNCTION__);
			add_log(array('table_id'=>'forms:'.$form_id, 'log_key'=>__FUNCTION__, 'log_text'=>'حالة النموذج "<b>'.$form_status->name.'</b>" تم تغييره.'));
			return true;
		} else { return false; }
	} else { return false; }
}
