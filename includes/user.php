...
function add_user($args) {
	...
	if(!have_log()) {
		...
		@form_validation($insert['email'], 'email', 'البريد الالكتروني', 'email', __FUNCTION__);
		@form_validation($insert['username'], 'username', 'الاسم', 'required|min_length[3]|max_length[20]', __FUNCTION__);
		@form_validation($insert['surname'], 'surname', 'الشهرة', 'required|min_length[3]|max_length[20]', __FUNCTION__);
		...
		if($q_is_email = db()->query("SELECT * FROM ".dbname('users')." WHERE username='".$insert['username']."' ")) {
			if($q_is_email->num_rows) {
				add_alert(_b($insert['username']).'  مستخدم من قبل موظف اخر .', 'warning', __FUNCTION__);
			}
		}
		...
		if($q_is_email = db()->query("SELECT * FROM ".dbname('users')." WHERE gsm='".$insert['gsm']."' ")) {
			if($q_is_email->num_rows) {
				add_alert(_b($insert['gsm']).' يتم استخدام رقم الجوال من قبل شخص آخر.', 'warning', __FUNCTION__);
			}
		}
		...
		if(!empty($insert['citizenship_no'])) {
			if($q_is_email = db()->query("SELECT * FROM ".dbname('users')." WHERE citizenship_no='".$insert['citizenship_no']."' ")) {
				if($q_is_email->num_rows) {
					add_alert(_b($insert['citizenship_no']).' رقم الهوية مستخدم سابقا.', 'warning', __FUNCTION__);
				}
			}
		}
		...
		if(isset($update['role'])) {
			if(get_active_user('role') > $insert['role']) {
				add_alert('نحن لسنا قادرين على تعيين سلطة أعلى مستوى.', 'danger', __FUNCTION__);
			}
		}
		...
		if(!is_alert(__FUNCTION__)) { // eger herhangi bir hata yok ise
			...
			if(db()->query("INSERT INTO ".dbname('users')." ".sql_insert_string($insert)." ")) {
				if($insert_id = db()->insert_id) {
					if($args['add_alert']) { add_alert('تم إنشاء حساب المستخدم.', 'success', __FUNCTION__); }
					if($args['add_log']) { add_log(array('table_id'=>'users:'.get_active_user_id(), 'log_key'=>'add_user', 'log_text'=>'تم إنشاء حساب مستخدم جديد.'._b($insert['username']).'')); }
					return $insert_id;
				} else { return false; }
...
function update_user($user_id, $args) {
	...
	if(!have_log()) { // bu islem daha onceden gercekles mi?
		...
		@form_validation($update['email'], 'email', 'البريد الاكتروني', 'email', __FUNCTION__);
		@form_validation($update['username'], 'username', 'الاسم', 'min_length[3]|max_length[20]', __FUNCTION__);
		@form_validation($update['surname'], 'surname', 'الشهرة', 'min_length[3]|max_length[20]', __FUNCTION__);
		@form_validation($update['gsm'], 'gsm', 'الهاتف المحمول', 'gsm', __FUNCTION__);
		...
		if(isset($update['username'])) {
			if($q_is_email = db()->query("SELECT * FROM ".dbname('users')." WHERE username='".$update['username']."' AND id NOT IN ('".$user_id."')  ")) {
				if($q_is_email->num_rows) {
					add_alert(_b($update['username']).' يتم استخدام عنوان البريد الإلكتروني من قبل مستخدم آخر.', 'warning', __FUNCTION__);
				}
			}
		}
		...
		if(isset($update['role'])) {
			if(get_active_user('role') > $update['role']) {
				add_alert('ليس لدينا سلطة تعيين سلطة أعلى مستوى.', 'danger', __FUNCTION__);
			}
		}
		...
		if(!is_alert(__FUNCTION__)) { // eger herhangi bir hata yok ise
			...
			if(db()->query("UPDATE ".dbname('users')." SET ".sql_update_string($update)." WHERE id='".$user_id."' ")) {
				if(db()->affected_rows > 0) {
					...
					if($args['add_alert']) { add_alert('تم تحديث حساب المستخدم.', 'success', __FUNCTION__); }
					if($args['add_log']) { add_log(array('table_id'=>'users:'.$user_id, 'log_key'=>'update_user', 'log_text'=>_b($old_user->display_name).' تم تحديث حساب المستخدم.', 'meta'=>log_compare($old_user, $new_user))); }
					return true;
				} else { return false; }
			} else { add_mysqli_error_log(__FUNCTION__); }
		} else { return false; }
	...
	if ( isset(til()->user[$where['id']]) AND $_til ) {
		...
		if($query = db()->query("SELECT * FROM ".dbname('users')." ".sql_where_string($where)." ")) {
			if($query->num_rows) {
				...
				if(!isset($return->display_name)) { $return->display_name = $return->username.' '.$return->surname; }
				til()->user[$return->id] = $return;
				return $return;
			} else {
				if($args['add_alert']) { add_alert('لم يتم العثور على حساب مستخدم يطابق المعايير التي تبحث عنها.', 'warning', __FUNCTION__); }
				return false;
			}
		} else { add_mysqli_error_log(__FUNCTION__); }
	}
...
function get_users($args=array()) {
	...
	if($query = db()->query("SELECT * FROM ".dbname('users')." ".sql_where_string($where)." ")) {
		if($query->num_rows) {
			...
			add_alert('لم يتم العثور على قائمة أعضاء.', 'warning', __FUNCTION__);
			return false;
		}
	} else {
...
function get_active_user($val='') {
	if($user = get_user($_SESSION['login_id'])) {
		...
		exit('لم يتم العثور على عضو دخول.');
	}
}
...
function add_user_meta($user_id, $meta_key, $meta_value, $args=array()) {
	...
	# kullanıcı kontrol
	if(!$user = get_user($user_id)) { add_console_log('#'.$user_id.' لم يتم العثور على المستخدم.', __FUNCTION__); return false; }
	if(empty($meta_key)) { add_console_log('لا يمكن أن يكون المفتاح ميتا فارغا.', __FUNCTION__); return false; }
...
function set_staff_salary($user_id) {
	...
	if(!@$user_meta['date_start_work']) { add_console_log('لم يتم العثور على تاريخ بدء العمل.', __FUNCTION__); return false; }
...
function get_user_role_text($val) {
	$text = '';
	if($val == '1') { $text = 'مدير مميز'; }
	if($val == '2') { $text = 'مدير'; }
	if($val == '3') { $text = 'مشرف متميز'; }
	if($val == '4') { $text = 'موظف معتمد'; }
	if($val == '5') { $text = 'موظف'; }
...
function _get_usermeta_school_text($text) {
	if($text == 'primary_education') {
		return 'تعليم ابتدائي';
	} elseif($text == 'high_school') {
		return 'المدرسة الثانوية';
	} elseif($text == 'college') {
		return 'كلية';
