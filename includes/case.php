...
function set_payment($id, $args=array()) {
	...
	if($insert['in_out'] != '0' and $insert['in_out'] != '1') { add_alert('نوع إخراج الإدخال غير صحيح', 'danger', __FUNCTION__); return false; }
	...
	if(!is_alert(__FUNCTION__)) {
		...
		if($id) {
			...
			if(update_payment($id, $_update)) {
				if($args['add_alert']) { add_alert('تم تحديث نموذج الدفع.', 'success', __FUNCTION__); }
				return true;
			} else {
				return false;
			}
		...
		if($old_form = get_form($id)) {
			if($q_update = db()->query("UPDATE ".dbname('forms')." SET ".sql_update_string($update)." WHERE id='".$id."'")) {
				if(db()->affected_rows) {
					...
					if($args['add_alert']) { add_alert('تم تحديث نموذج الدفع.', 'success', __FUNCTION__); }
					if($args['add_log']) { add_log(array('uniquetime'=>@$args['uniquetime'], 'table_id'=>'forms:'.$id, 'log_key'=>__FUNCTION__, 'log_text'=>'تم تحديث نموذج الدفع.', 'meta'=>log_compare($old_form, $new_form))); }
					return true;
				} else { return false; }
			} else { add_mysqli_error_log(__FUNCTION__); }
		} else { return false; }
	...
	if(!is_alert(__FUNCTION__)) {
		if($case_id = add_extra($_insert)) {
			if(@$_insert['insert']['val_enum']) { // is_bank
				if($args['add_alert']) { add_alert(_b($_insert['insert']['name']).' تم إنشاء حساب مصرفي.', 'success', __FUNCTION__); }
				if($args['add_log']) { add_log(array('uniquetime'=>@$args['uniquetime'], 'table_id'=>'extra:'.$case_id, 'log_key'=>__FUNCTION__, 'log_text'=>_b($_insert['insert']['name']).' تم إنشاء حساب مصرفي.')); }
			} else {
				if($args['add_alert']) { add_alert(_b($_insert['insert']['name']).' تم أنشاؤه.', 'success', __FUNCTION__); }
				if($args['add_log']) { add_log(array('uniquetime'=>@$args['uniquetime'], 'table_id'=>'extra:'.$case_id, 'log_key'=>__FUNCTION__, 'log_text'=>_b($_insert['insert']['name']).' تم إنشاؤه.')); }
			}
		} else { return false; }
	}
