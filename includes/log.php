...
function get_logs($query) {
	...
	if($query = db()->query($query_str)) {
		if($query->num_rows) {
			...
			add_alert('لم يتم العثور على سجل السجل.', 'warning', 'get_logs');
			return false;
		}
	} else {
		add_alert('<b>خطأ في قاعدة البيانات:</b> '.db()->error, 'خطر', 'get_logs');
		return false;
	}
}
...
function repetitive_operation($function_name, $message='', $class='warning') {
	if(empty($message)) {
		$message = 'تم اجراء العملية بالفعل. ';
	}
...
function add_console_log( $args, $function_name=false ) {
	...
	if(!$function_name) { $function_name = 'ultra'; }
    if ( is_array( $args ) )
