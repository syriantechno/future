    ...
    if (!have_log()) { // إذا لم يتم إضافة إيغر من قبل، لن يتم العثور على إدخال السجل.
        ...
        if (!is_alert(__FUNCTION__, 'danger')) { // eger herhangi bir hata yok ise
            ...
            if ($q_is_code->num_rows) { // eger veritabanında ayni barkod var ise
                ...
                if (isset($insert['city'])) {
                    $insert['city'] = til_get_strtoupper($insert['city']);
                }
                ...
                if (isset($insert['tax_home'])) {
                    $insert['tax_home'] = til_get_strtoupper($insert['tax_home']);
                }
...
function update_account($where, $args)
{
    ...
    if (isset($update['id'])) {
        unset($update['id']);
    } // guvenlik icin eger hesap ID guncellenmek isteniyorsa guncellenmesin
