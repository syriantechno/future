...
function add_account($args) {
	...
	if(!have_log()) { // إذا لم يتم إضافة إيغر من قبل، لن يتم العثور على إدخال السجل.
        ...
        @form_validation($insert['fathername'], 'fathername', 'اسم الاب', 'min_length[2]|max_length[100]', __FUNCTION__);
		@form_validation($insert['countrynumber'], 'countrynumber', ' الرقم الوطني', 'min_length[2]|max_length[25]', __FUNCTION__);
		@form_validation($insert['gsm'], 'gsm', 'الهاتف المحمول', 'gsm', __FUNCTION__);
		@form_validation($insert['Retardationnum'], 'Retardationnum', 'رقم بطاقة الاعاقة', 'number|min_length[1]|max_length[25]', __FUNCTION__);
		...
		@form_validation($insert['mo3el'], 'mo3el', 'المعيل', 'min_length[3]|max_length[25]', __FUNCTION__);
		...
		if(!is_alert(__FUNCTION__, 'danger')) { // eger herhangi bir hata yok ise
			...
			if($q_is_code->num_rows) { // eger veritabanında ayni barkod var ise
				...
				if(db()->query("INSERT INTO ".dbname('accounts')." ".sql_insert_string($insert)." ")) {
					...
					if($args['add_alert']) { add_alert('تمت إضافة بطاقة حساب جديدة.', 'success', 'add_account'); }
					if($args['add_log'])	{ add_log(array('uniquetime'=>@$insert['uniquetime'], 'table_id'=>'accounts:'.$insert_id, 'log_key'=>'add_account', 'log_text'=>'تمت إضافة بطاقة الحساب.')); }
					return $insert_id;
...
function get_account($args, $_til=true) {
	...
	if($query = db()->query("SELECT * FROM ".dbname('accounts')." ".sql_where_string($where)." ")) {
		if($query->num_rows) {
			...
			if($args['add_alert']) { add_alert('لم يتم العثور على بطاقة حساب تطابق المعايير التي تبحث عنها.', 'warning', __FUNCTION__); }
			return false;
		}
	} else { add_mysqli_error_log(__FUNCTION__); }
} //.get_account()
...
function update_account($where, $args) {
	...
	if(!have_log()) { // إذا لم يتم إضافة إيغر من قبل، لن يكون هناك إدخالات السجل.
		if($old_account = get_account($where)) {
            ...
            @form_validation($update['code'], 'code', 'رمز الباركود', 'min_length[3]|max_length[32]', 'update_account');
            @form_validation($update['name'], 'name', 'اسم الحساب', 'required|min_length[3]|max_length[32]', 'update_account');
            @form_validation($update['fathername'], 'fathername', 'اسم الاب', 'min_length[2]|max_length[100]', 'update_account');
            @form_validation($update['countrynumber'], 'countrynumber', ' الرقم الوطني', 'min_length[2]|max_length[25]', 'update_account');
            @form_validation($update['gsm'], 'gsm', 'الهاتف المحمول', 'gsm', 'update_account');
            @form_validation($update['Retardationnum'], 'Retardationnum', 'رقم بطاقة الاعاقة', 'number|min_length[1]|max_length[25]', 'update_account');
            @form_validation($update['address'], 'address', 'العنوان', 'min_length[3]|max_length[255]', 'update_account');
            @form_validation($update['district'], 'district', 'مقاطعة', 'min_length[3]|max_length[25]', 'update_account');
            @form_validation($update['city'], 'city', 'المدينة', 'min_length[3]|max_length[25]', 'update_account');
            @form_validation($update['Retardationtype'], 'Retardationtype', 'نوع الاعاقة', 'min_length[3]|max_length[25]', 'update_account');
            @form_validation($update['mo3el'], 'mo3el', 'المعيل', 'min_length[3]|max_length[25]', 'update_account');
				...
				if($q_is_code->num_rows) { // إذا كان قاعدة البيانات إغير لديه نفس الرمز الشريطي
					...
					if(db()->query("UPDATE ".dbname('accounts')." SET ".sql_update_string($update)." WHERE id='".$old_account->id."' ")) {
						if(db()->affected_rows > 0) {
							...
							if($args['add_alert']) { add_alert('تم تحديث بطاقة الحساب.', 'success', 'update_account'); }
							if($args['add_log']) { add_log(array('table_id'=>'accounts:'.$old_account->id, 'log_key'=>'update_account', 'log_text'=>'تم تحديث بطاقة الحساب.', 'meta'=>log_compare($old_account, $new_account))); }
							return true;
						} else { return false; }
					} else { add_mysqli_error_log(__FUNCTION__); }
				}
...
function get_accounts_delete($args=array()) {
    $return = new StdClass;
    ...
    // required
    $args = get_ARR_helper_limit_AND_orderby($args);
    if(!isset($args['status'])) 	{ $args['status'] = '0'; } else { $args['status'] = input_check($args['status']); }
