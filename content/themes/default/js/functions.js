           ...
           if ( !is_mobile() ) {
              getXHR({'method': 'get', 'url': get_site_url('admin/user/lastNotificationInfo.php?session_id='+window.session_id)}, function(last_notification) {
                if ( last_notification != false && last_notification != 'empty' ) {
                  ...
                  browser_notification({title: 'ultra', content: object.title, url: object.message, id: object.id});
                }
              });
            }
          }
...
function imageHandler(image, callback) {
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        if ( xhr.responseText == "") {
          console.log("تأكد من أن الملف ليس أكبر من 5 ميغابايت");
          callback(false);
        } else {
...
function calendar(selector, events) {
  ...
  $this.fullCalendar({
    ...
    locale: 'tr',
    views: {
      listDay: { buttonText: 'يوميا' },
      listWeek: { buttonText: 'أسبوعيا' }
    },
    header: {
      left: 'prev,next',
      center: center,
