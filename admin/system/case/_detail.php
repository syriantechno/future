	...
	<div class="row">
		<div class="col-md-4">
			<form name="form_status" id="form_status" action="" method="POST" class="validate">
				...
				<div class="form-group">
					<label class="veritical-center"><input type="checkbox" name="default" id="default" value="true" class="toogle" <?php checked($case->is_default, 'default'); ?> data-size="mini" data-on-text="Evet" data-off-text="Hayır"> &nbsp; يتم تحديد الافتراضي عند إنشاء النموذج الأول؟</label>
				</div> <!-- /.form-group -->
			  ...
			  <form name="status_delete" id="status_delete" action="" method="GET" class="modal-dialog modal-md" role="document">
			    <div class="modal-content panel-danger">
			      ...
			      <div class="modal-body">
			        ...
			        <div class="alert <?php echo ($case->count > 0)? 'alert-danger' : 'alert-warning'; ?>"><?php echo $case->name; ?> ينتمون إلى، <?php echo _b($case->count); ?> تم العثور على نموذج دفع الوحدة.</div>
			        ...
			        <?php if($case->count > 0): ?>
			        	<?php if($form_status_all): ?>
				        	<p><small class="text-muted">يرجى تحديد حالة نموذج مختلف من القائمة أدناه. <b><?php echo $case->name; ?></b> form durumuna ait <?php echo _b($case->count); ?> وسوف يتغير شكل جهاز الكمبيوتر الشخصي مع وضع النموذج الذي تحدده أدناه.</small></p>
				        	...
				        	<div class="h-10"></div>
				        	<p class="text-danger"><b><?php echo $case->name; ?></b> لحذف حالة النموذج، و <?php echo _b($case->count); ?> هل تريد نقل النموذج إلى النموذج الجديد الذي حددته أعلاه؟</p>
					    <?php else: ?>
