<?php add_page_info( 'nav', array('name'=>'الصندوق والبنك' ) ); ?>
<?php
...
if(isset($_POST['add'])) {
	...
	@form_validation($_POST['name'], 'status_name', 'اسم الصندوق', 'required|min_length[3]|max_length[64]', 'input_control_');
...
<div class="row">
	<div class="col-lg-6 col-md-6">
		<form name="form_status" id="form_status" action="<?php set_url_parameters(array('add'=>array('taxonomy'=>$_taxonomy) )); ?>" method="POST" class="validate">
			...
			<div class="form-group">
				<label for="name">اسم الصندوق او البنك</label>
			...
			<div class="form-group">
				<label for="status_name">صندوق او بنك</label>
			...
			<div class="bank_detail bg-info padding-5 margin-bottom-15">
				...
				<div class="form-group">
					<label for="bank_admin_name">اسم صاحب حساب البنك</label>
	...
	<div class="col-lg-6 col-md-6">
		...
		<div class="panel panel-default">
			<div class="panel-heading"><h3 class="panel-title">الصناديق</h3></div>
			<div class="panel-body">
				...
				<?php if($cases): ?>
					<table class="table table-hover table-condensed">
						...
						<?php foreach($cases as $case):  $case->total = calc_case($case->id); ?>
							<tr>
								<td><a href="?detail&status_id=<?php echo $case->id; ?>"><span><?php echo $case->name; ?></span></a> <?php if($case->is_default == 'default'): ?><small class="text-muted">(الافتراضي)</small><?php endif; ?></td>
				...
				<?php else: ?>
					<?php echo get_alert('لم يتم العثور على الصندوق.', 'warning', false); ?>
				<?php endif; ?>
			</div> <!-- /.panel-body -->
		</div> <!-- /.panel -->
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<?php if($banks): ?>
					<table class="table table-hover table-condensed">
						...
						<?php foreach($banks as $bank): $bank->total = calc_case($bank->id); ?>
							<tr>
								<td><a href="?detail&status_id=<?php echo $bank->id; ?>"><span><?php echo $bank->name; ?></span></a> <?php if($bank->is_default == 'default'): ?><small class="text-muted">(الافتراضي)</small><?php endif; ?></td>
				...
				<?php else: ?>
					<?php echo get_alert('لم يتم العثور على البنك.', 'warning', false); ?>
				<?php endif; ?>
			</div> <!-- /.panel-body -->
		</div> <!-- /.panel -->
