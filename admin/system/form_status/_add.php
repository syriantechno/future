<?php add_page_info( 'nav', array('name'=>$_name.' جديد' ) ); ?>
<?php
...
if(isset($_POST['add'])) {
	...
	@form_validation($_POST['status_name'], 'status_name', 'اسم الحالة', 'required|min_length[3]|max_length[64]', 'input_control_');
		...
		<form name="form_status" id="form_status" action="<?php set_url_parameters(array('add'=>array('taxonomy'=>$_taxonomy) )); ?>" method="POST" class="validate">
			...
			<?php if($_bg_color or $_color): ?>
				<div class="form-group text-center">
					<span class="label label-success label-fs-example long" style="background-color:#fdc430; color:#000000;">معاينة</span>
					|
					<span class="label label-success label-fs-example abbreviation" style="background-color:#fdc430; color:#000000;">OD</span>
				</div> <!-- /.form-group -->
				<script>
				...
				$(document).ready(function() {
					$('#status_name').keyup(function() {
						...
						if(text.length > 0) {
							...
							$('.label-fs-example.long').html('معاينة');
							$('.label-fs-example.abbreviation').html('مع');
						}
					});
			...
			<div class="row">
				<?php if($_bg_color): ?>
					<div class="col-md-6">
						<div class="form-group">
							<label for="status_bg_color"><i class="fa fa-square color-bg-status" style="color:#fdc430;"></i> كود لون الخلفية</label>
							<input type="text" name="status_bg_color" id="status_bg_color" class="form-control required colorpicker_bg_status"  maxlength="32" value="#fdc430">
							<script>
							$(document).ready(function() {
				...
				<?php if($_color): ?>
					<div class="col-md-6">
						<div class="form-group">
							<label for="status_color"><i class="fa fa-square color-status" style="color:#000;"></i> كود لون النص</label>
							<input type="text" name="status_color" id="status_color" class="form-control required colorpicker_status"  maxlength="32" value="#000">
							<script>
							$(document).ready(function() {
			...
			<?php if($_sms_template): ?>
				<div class="form-group">
					<label for="sms_template">قالب رسالة نصية <small> <span id="sms_count">0</span>  حرف</small></label>
					<label class="veritical-center pull-right"><small class="text-muted">هل تريد ارسال رسالة نصية تلقائية</small> &nbsp; <input type="checkbox" name="auto_sms" id="auto_sms" value="true" class="toogle" data-size="mini" data-on-text="نعم" data-off-text="لا"></label>
					<textarea name="sms_template" id="sms_template" class="form-control fs-12 h-100">[form_id] حالة طلبك [form_status] حالة النموذج.</textarea>
				</div> <!-- /.form-group -->
				<script>
				$(document).ready(function() {
			...
			<?php if($_email_template): ?>
				<div class="form-group">
					<label for="email_template">نموذج البريد الالكتروني</label>
					<label class="veritical-center pull-right"><small class="text-muted">هل تريد ارسال بريد الكتروني تلقائي</small> &nbsp; <input type="checkbox" name="auto_email" id="auto_email" value="true" class="toogle" data-size="mini" data-on-text="نعم" data-off-text="لا"></label>
				...
				<?php if($form_status): ?>
					<table class="table table-hover table-condensed">
						<thead>
							<tr>
								<th>اسم الحالة</th>
								<?php if($_email_template): ?><th width="100">بريد تلقائي</th><?php endif; ?>
								<?php if($_sms_template): ?><th width="100">رسالة نصية تلقائية</th><?php endif; ?>
							</tr>
						</thead>
						<tbody>
						<?php foreach($form_status as $status): ?>
							<tr>
								<td><a href="?taxonomy=<?php echo $_taxonomy; ?>&detail&status_id=<?php echo $status->id; ?>"><span class="<?php if($status->bg_color): ?>label label-success<?php endif; ?>" style="background-color:<?php echo $status->bg_color; ?>; color:<?php echo $status->color; ?>;"><?php echo $status->name; ?></span></a> <?php if($status->is_default == 'default'): ?><small class="text-muted">(افتراضي)</small><?php endif; ?></td>
								<?php if($_email_template): ?><td class="text-center"><?php if($status->auto_email == 'auto_email'): ?>نعم<?php else: ?> لا <?php endif; ?></td><?php endif; ?>
								<?php if($_sms_template): ?><td class="text-center"><?php if($status->auto_sms == 'auto_sms'): ?>نعم<?php else: ?> لا <?php endif; ?></td><?php endif; ?>
							</tr>
						<?php endforeach; ?>
						</tbody>
					</table>
