...
<?php if($status = get_form_status(array('id'=>$_GET['status_id'], 'taxonomy'=>$_taxonomy))):?>
	...
	<div class="row">
		<div class="col-md-4">
			<form name="form_status" id="form_status" action="<?php set_url_parameters(array('add'=>array('taxonomy'=>$_taxonomy) )); ?>" method="POST" class="validate">
				...
				<div class="row">
					<?php if($_bg_color): ?>
						<div class="col-md-6">
							<div class="form-group">
								<label for="status_bg_color"><i class="fa fa-square color-bg-status" style="color:<?php echo $status->bg_color; ?>;"></i> رمز لون الخلفية</label>
					...
					<?php if($_color): ?>
						<div class="col-md-6">
							<div class="form-group">
								<label for="status_color"><i class="fa fa-square color-status" style="color:<?php echo $status->color; ?>;"></i> رمز لون الخط</label>
				...
				<?php if($_sms_template): ?>
					<div class="form-group">
						<label for="sms_template">قالب الرسائل النصية <small>(<span id="sms_count">0</span> حرف)</small></label>
						<label class="veritical-center pull-right"><small class="text-muted">هل تريد ارسال رسالة نصية تلقائية?</small> &nbsp; <input type="checkbox" name="auto_sms" id="auto_sms" value="true" class="toogle" <?php checked($status->auto_sms, 'auto_sms'); ?> data-size="extramini" data-on-text="Evet" data-off-text="لا"></label>
				...
				<?php if($_email_template): ?>
					<div class="form-group">
						...
						<label class="veritical-center pull-right"><small class="text-muted">البريد الإلكتروني إرسال تلقائي؟</small> &nbsp; <input type="checkbox" name="auto_email" id="auto_email" value="true" class="toogle" <?php checked($status->auto_mail, 'auto_email'); ?> data-size="extramini" data-on-text="Evet" data-off-text="Hayır"></label>
				...
				<div class="form-group">
					<label class="veritical-center"><input type="checkbox" name="default" id="default" value="true" class="toogle" <?php checked($status->is_default, 'default'); ?> data-size="mini" data-on-text="Evet" data-off-text="Hayır"> &nbsp; هل يجب تحديده النموذج الافتراضي؟</label>
				</div> <!-- /.form-group -->
			...
			<br />
			<label>هل تريد حذف حالة النموذج? <a href="#" class="text-danger" data-toggle="modal" data-target="#form_status_delete"><i class="fa fa-trash"></i> نعم، حذف النموذج</a></label>
			  ...
			  <form name="status_delete" id="status_delete" action="" method="GET" class="modal-dialog modal-md" role="document">
			    <div class="modal-content panel-danger">
			      ...
			      <div class="modal-body">
			        ...
			        <div class="alert <?php echo ($status->count > 0)? 'alert-danger' : 'alert-warning'; ?>"><?php echo $status->name; ?> من الشكل, <?php echo _b($status->count); ?> وجدت شكل قطع.</div>
			        ...
			        <?php if($status->count > 0): ?>
			        	<?php if($form_status_all): ?>
				        	<p><small class="text-muted">يرجى تحديد حالة نموذج مختلف من القائمة أدناه. <b><?php echo $status->name; ?></b> من الشكل <?php echo _b($status->count); ?> سيتغير عدد النماذج وفقًا لحالة النموذج الجديدة التي تختارها أدناه..</small></p>
				        	...
				        	<div class="h-10"></div>
				        	<p class="text-danger"><b><?php echo $status->name; ?></b> form لحذف حالة هذا النموذج <?php echo _b($status->count); ?> هل تريد نقل النموذج إلى شكل النموذج الجديد أعلاه?</p>
					    <?php else: ?>
				        		<p><small class="text-muted"><b><?php echo $status->name; ?></b>لا يمكنك حذف حالة النموذج. <br /> لأنه في شكل <?php echo _b($status->count); ?> لا يمكن العثور على أي وضع شكل آخر. </small></p>
						<?php endif; ?>
					<?php else: ?>
						<?php if($form_status_all): ?>
							<p><small class="text-muted"><b><?php echo $status->name; ?></b> هل تريد حذف حالة النموذج؟ </small></p>
						<?php else: ?>
							<div class="alert alert-danger">يتم وضع حالة النموذج بشكل افتراضي. لا يمكنك حذف حالة النموذج.</div>
						<?php endif; ?>
					<?php endif; ?>
			      ...
			      <div class="modal-footer">
			        ...
			        <?php if($form_status_all): ?><button type="submit" class="btn btn-primary">نعم أقبل</button><?php endif; ?>
			      </div> <!-- /.modal-footer -->
			    </div> <!-- /.modal-content -->
			  </form>
			</div> <!-- /.modal -->
