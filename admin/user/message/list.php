...
if(isset($_GET['box'])) {
	$box = 'inbox';
	if($_GET['box'] == 'inbox') { $box = 'inbox'; add_page_info('title', 'صندوق الرسائل - الوارد'); }
	elseif($_GET['box'] == 'outbox') { $box = 'outbox'; add_page_info('title', 'صندوق الرسائل - المحفوظة'); }
	elseif($_GET['box'] == 'trash') { $box = 'trash'; add_page_info('title', 'صندوق الرسائل - المحذوفة'); }
} else {
	$box = 'inbox';
}
				...
				<table class="table table-hover table-condensed table-striped dataTable">
					...
					<?php foreach($messages as $list): ?>
						...
						<tr class="<?php if(!$list->read_it AND $list->inbox_u_id == get_active_user('id') AND $box != 'outbox'): ?>bold<?php endif; ?> pointer" onclick="location.href='<?php site_url('message', $list->id); ?>';">
							...
							<td width="100" class="text-right hidden-xs-portrait"><?php echo get_time_late($list->date_update); ?> منذ</td>
						</tr>
					<?php endforeach; ?>
					</tbody>
				</table>
