...
if(isset($_GET['id'])) {
	if($message = get_message($_GET['id'])) {
		...
		echo get_alert(_b($_GET['id']).' في قاعدة بيانات معرف الرسالة <u>bulunamadı</u>.', 'danger', false); get_footer(); exit;
	}
...
// مربع الاختيار لمربع الرسالة المحدد
...
add_page_info( 'nav', array('name'=>'صندوق الرسائل', 'url'=>get_site_url('admin/user/message/list.php?box='.$box) ) );
add_page_info( 'nav', array('name'=>$rec_user->username.' '.$rec_user->surname) );
												...
												<div class="message-elem-content">
													<div class="well padding-10 br-3">
														<div class="text-muted fs-11 italic">
															<span class="inform-text">أرسلت قبل.</span><span class="bold date-tooltip" <?php if ( til_is_mobile() ) { echo 'data-wenk-pos="left"'; } ?> data-wenk="<?php echo substr($message->date,0,16); ?>" title="<?php echo substr($message->date,0,16); ?>"><?php echo get_time_late($message->date); ?></span> <span class="inform-text">عن طريق</span> <span class="bold username"><?php echo get_user_info($message->sen_u_id, 'name'); ?> <?php echo get_user_info($message->sen_u_id, 'surname'); ?></span>  
														</div><!--/ .text-muted /-->
								...
								<div class="col-md-11 col-xs-12">
									...
									<?php else: ?>
										<div class="form-group message-area">
											...
											<label for="message" class="text-muted"><?php echo _b($rec_user->name.' '.$rec_user->surname); ?> اكتب رسالة لإرسالها...</label>
									...
									<div class="form-group hidden-xs">
										<button type="submit" class="btn btn-default pull-right hidden-xs"><i class="fa fa-send-o"></i> ارسال</button>
