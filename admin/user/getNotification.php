...
<?php
	...
	if ( $_GET['query_type'] == 'count' ) {
      ...
      echo get_calc_task();
		...
		if ( $_GET['box'] == 'message' ) : ?>
			<li class="message-header"><?php echo get_calc_message(); ?> رسالة غير مقروءة</li>
		  ...
		  <?php endif; // if( $messages = get_messages(array('query'=>_get_query_message('inbox').' LIMIT 5')) ) ?>
			...
			<li class="message-footer"><a href="<?php site_url('admin/user/'. $_GET['box'] .'/list.php'); ?>">عرض الكل</a></li>
		...
		// get task list
		if ( $_GET['box'] == 'task' ) : ?>
			<li class="message-header"><?php echo get_calc_task(); ?> مهمة غير مقروءة</li>
		...
		if ( $_GET['box'] == 'notification' ) : ?>
			<li class="message-header">
				<?php echo get_calc_notification(); ?> إخطار غير مقروء
		  ...
		  <?php else: ?>
		    <div class="p-5">
		      <?php echo get_alert('مربع الملاحظات فارغ', 'warning', false); ?>
		    </div>
		  <?php endif; ?>

		  <?php if ( !til_is_mobile() ): ?><li class="message-footer"><a href="<?php site_url('admin/user/'. $_GET['box'] .'/list.php'); ?>">عرض الكل</a></li><?php endif; ?>
		<?php endif;
	} else { exit; }
?>
