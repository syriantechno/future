...
if( !$user = get_user($_GET['id']) ) { echo get_alert('لم يتم العثور على حساب المستخدم.', 'warning', false); get_footer(); return false; }
...
<ul class="nav nav-tabs" role="tablist"> 
	<li role="presentation" class="active"><a href="#home" id="home-tab" role="tab" data-toggle="tab" aria-controls="home" aria-expanded="true"><i class="fa fa-user-o"></i> الملف الشخصي</a></li> 
	<?php if(user_access('admin')): ?><li role="presentation"><a href="#cv" id="cv-tab" role="tab" data-toggle="tab" aria-controls="cv" aria-expanded="true"><i class="fa fa-address-book-o"></i> السيرة الذاتية</a></li><?php endif; ?>
	<?php if(user_access('admin')): ?><li role="presentation"><a href="#salary" id="salary-tab" role="tab" data-toggle="tab" aria-controls="salary" aria-expanded="true"><i class="fa fa-money"></i> راتب</a></li><?php endif; ?>
	<li role="presentation" class=""><a href="#logs" role="tab" id="logs-tab" data-toggle="tab" aria-controls="logs" aria-expanded="false"><i class="fa fa-database"></i> تاريخ</a></li> 

	<li role="presentation" class="dropdown pull-right"> <a href="#" class="dropdown-toggle" id="myTabDrop1" data-toggle="dropdown" aria-controls="myTabDrop1-contents" aria-expanded="false"><i class="fa fa-cogs"></i> خيارات <span class="caret"></span></a> 
		<ul class="dropdown-menu" aria-labelledby="myTabDrop1" id="myTabDrop1-contents"> 
			<li><a href="#dropdown1" role="tab" id="dropdown1-tab" data-toggle="tab" aria-controls="dropdown1">تعيين المهمة</a></li> 
			<li><a href="<?php site_url('admin/user/message/add.php?rec_u_id='.$user->id); ?>">ارسل رسالة</a></li> 
		</ul> 
	</li>
	<li role="presentation" class="dropdown pull-right"> <a href="#" class="dropdown-toggle" id="myTabDrop1" data-toggle="dropdown" aria-controls="myTabDrop1-contents" aria-expanded="false"><i class="fa fa-print"></i> طباعة <span class="caret"></span></a> 
		<ul class="dropdown-menu" aria-labelledby="myTabDrop1" id="myTabDrop1-contents"> 
			<li><a href="print_cv.php?id=<?php echo $user->id; ?>" target="_blank">السيرة الذاتية (CV) طباعة</a></li> 
			<li class="divider"></li>
			<li><a href="print_salary.php?id=<?php echo $user->id; ?>" target="_blank">الراتب طباعة طباعة</a></li> 
			<li><a href="print_salary_detail.php?id=<?php echo $user->id; ?>" target="_blank">طباعة بيان الراتب المفصل</a></li> 
		</ul> 
	</li>
</ul>
	...
	<div class="tab-pane active fade in" role="tabpanel" id="home" aria-labelledby="home-tab"> 
		...
		<div class="row">
			<div class="col-md-6">
				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							...
							<input type="text" name="email" id="email" value="<?php echo $user->email; ?>" class="form-control email">
						</div> <!-- /.form-group -->
					</div> <!-- /.col-md-6 -->
					<div class="col-md-6">
						<div class="form-group">
							<label for="gsm">الهاتف المحمول</label>
							<input type="text" name="gsm" id="gsm" value="<?php echo $user->gsm; ?>" class="form-control digits" minlength="3" maxlength="11">
						</div> <!-- /.form-group -->
					</div> <!-- /.col-md-6 -->
				</div> <!-- /.row -->
				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							...
							<input type="text" name="username" id="username" value="<?php echo $user->username; ?>" class="form-control" minlength="3" maxlength="25">
						</div> <!-- /.form-group -->
					</div> <!--- /.col-md-6 -->
					<div class="col-md-6">
						<div class="form-group">
				...
				<div class="col-md-6">
					<div class="form-group">
						...
						<input type="text" name="citizenship_no" id="citizenship_no" class="form-control digits" minlength="3" maxlength="25" value="<?php echo $user->citizenship_no; ?>">
					</div> <!-- /.form-group -->
				</div> <!-- /.col-md-6 -->
			</div> <!-- /.row -->
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select name="role" id="role" class="select form-control" <?php if(!user_access('admin')): echo 'disabled'; endif; ?>>
									...
									<option value="4" <?php selected($user->role, '4'); ?>>كبار الموظفين</option>
									<option value="3" <?php selected($user->role, '3'); ?>>مشرف قسم</option>
									<option value="2" <?php selected($user->role, '2'); ?>>مدير</option>
									<?php if(user_access('superadmin')): ?><option value="1" <?php selected($user->role, '1'); ?>>مدير مميز</option><?php endif; ?>
								</select>
							</div> <!-- /.form-group -->
						</div> <!-- /.col-md-6 -->
					</div> <!-- /.row -->
			...
			<div class="col-md-2">
				...
				<div class="img-thumbnail">
					...
					<?php if(user_access('admin')): ?>
						<div class="h-10"></div>
						<a href="?id=<?php echo $user->id; ?>&delete_avatar" class="pull-right text-danger fs-12"><i class="fa fa-trash"></i> حذف هذه الصورة</a>
					<?php endif; ?>
				</div>
		...
		<?php if(user_access('admin')): ?>
			...
			<?php if($user->status == 1): ?>
				...
				<a href="#" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#userDelete"><i class="fa fa-trash-o"></i> نعم، حذف</a>
				  ...
				  <div class="modal-dialog" role="document">
				    <div class="modal-content">
				      ...
				      <div class="modal-body">
				        <p><strong><?php echo $user->display_name; ?></strong> هل تريد حذف حساب المستخدم?</p>

				        <small class="text-muted">لا يتم حذف حساب المستخدم من قاعدة البيانات، يتم حظر الحساب فقط من دخول النظام. لا يمكن لصفحات مثل "جميع المستخدمين" الدخول إلى حساب المستخدم هذا. </small>
				      </div>
				      <div class="modal-footer">
				        <button type="button" class="btn btn-default" data-dismiss="modal">نعم</button>
				        <a href="?id=<?php echo $user->id; ?>&status=0" class="btn btn-danger">نعم، أوافق</a>
				      </div>
				    </div>
				  </div>
				</div>
			<?php else: ?>
				...
				<a href="?id=<?php echo $user->id; ?>&status=1" class="btn btn-success btn-xs"><i class="fa fa-undo"></i> نعم، تفعيل</a>
			<?php endif; ?>
		<?php endif; ?>
