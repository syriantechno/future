		...
		<tbody>
			<?php foreach($users as $list): ?>
				<tr onclick="window.location = '<?php site_url('admin/user/user.php'); ?>?id=<?php echo $list->id; ?>'; " class="pointer">
					<td><img src="<?php echo get_user_info($list->id, 'avatar'); ?>" class="img-responsive img-32 br-5 inline-block"> <?php echo $list->username; ?> <?php echo $list->surname; ?></td>
					<td class="hidden-xs-portrait"><a href="<?php site_url('admin/user/user.php'); ?>?id=<?php echo $list->id; ?>"><?php echo $list->email; ?></a></td>
