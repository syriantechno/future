...
<?php if (!isset($_GET['id'])) {
    echo get_alert('مطلوب معرف المهمة.', 'danger', false);
    get_footer();
    exit;
} ?>
...
if (isset($_GET['id'])) {
    ...
    if ($task = get_task($_GET['id'])) {
        ...
        echo get_alert('لم يتم العثور على معرف المهمة في قاعدة البيانات.', 'danger', false);
        get_footer();
        exit;
    }
...
if (isset($_POST['save_choice'])) {
    ...
    foreach ($choice as $id => $key) {
        if (isset($_POST['choice_' . $id])) {
            ...
            if ($is_choice = json_decode(get_task($_GET['id'])->choice)) {
                if ($is_choice->$id->is_it_done == '1') {
                    $_choice[$id]['date'] = $is_choice->$id->date;
                }
            }


            if (!$choice->$id->is_it_done) {
                $_message['insert']['message'] = '"<small class="text-muted italic">' . $choice->$id->text . '</small>" مهمة ' . _b('مكتملة') . ' وتمت الاضافة الى شريط التقدم.';
            ...
            if ($choice->$id->is_it_done) {
                $_message['insert']['message'] = '"<small class="text-muted italic">' . $choice->$id->text . '</small>" مهمة ' . _b('مصنوع') . ' وتمت الاضافة الى شريط التقدم.';
                $_message['add_alert'] = false;
                $_message['uniquetime'] = get_uniquetime();
                add_task_message($task->id, $_message);
            }
    ...
    if (db()->query("UPDATE " . dbname('messages') . " SET choice='" . $_choice . "'  WHERE id='" . $task->id . "'")) {
        if (db()->affected_rows) {
            add_alert('تم تحديث خيارات المهمة.', 'success');
            ...
            if ($task->type_status == '1' and $task->choice_open) {
                ...
                if (update_task($task->id, $_task)) {
                    add_alert('ونظرا لأنه تم العثور على المهمة غير المكتملة، أعيد فتح المهمة.', 'warning');
                    $task = get_task($task->id);
                }
            }
        }
...
if (isset($_POST['update_detail']) and $task->sen_u_id == get_active_user('id')) {
    ...
    if (update_task($task->id, $_task)) {
        ...
        if ($_task['date_end'] . ':00' != $task->date_end) {
            $_message = array();
            $_message['insert']['message'] = 'تاريخ إكمال الواجب ' . _b($_task['date_end']) . ' وقال انه يتغير.';
...
if (isset($_GET['type_status'])) {
    if ($_GET['type_status'] == '0' or $_GET['type_status'] == '1') {
        ...
        if ($_GET['type_status'] == '1' and $task->choice_open) { // gorev kapatiliyor ve acikta gorev secenegi var ise
            add_alert('لا يمكن إيقاف هذه المهمة بسبب وجود خيارات مهمة غير مفتوحة.', 'warning', 'type_status');
        } else {
            $_task = array();
            ...
            if (update_task($task->id, $_task)) {
                ...
                if ($_GET['type_status'] == '0') {
                    $_message['insert']['message'] = 'تم تفعيل هذه المهمة مرة أخرى من قبلي.';
                } elseif ($_GET['type_status'] == '1') {
                    $_message['insert']['message'] = 'وقد أغلقني هذه المهمة.';
                }
        ...
        add_alert('حالة البعثة فقط "0" أو "1" قد يكون قيمة.', 'warning', 'type_status');
    }
}


# secili olan mesaj icin $box degiskenine deger atayalim
if ($task->rec_u_id == get_active_user('id')) {
    $box = 'inbox';
}
if ($task->sen_u_id == get_active_user('id')) {
    $box = 'outbox';
}
if ($task->sen_trash_u_id == get_active_user_id('id') or $task->rec_trash_u_id == get_active_user_id('id')) {
    $box = 'trash';
}

add_page_info('title', $task->title);
add_page_info('nav', array('name' => 'مربع المهام', 'url' => get_site_url('admin/user/task/list.php?box=' . $box)));
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label for="sen_user_display_name">مرسل المهمة</label>
                                <input type="text" name="sen_user_display_name" id="sen_user_display_name"
                                       class="form-control" value="<?php user_info($task->sen_u_id, 'display_name'); ?>"
                                       readonly>
                            </div> <!-- /.form-group -->

                            <div class="form-group">
                                <label for="rec_user_display_name">متلقي المهمة</label>
                                <input type="text" name="rec_user_display_name" id="rec_user_display_name"
                                       class="form-control" value="<?php user_info($task->rec_u_id, 'display_name'); ?>"
                                       readonly>
                            </div> <!-- /.form-group -->

                            <div class="row space-5" style="margin-top: 35px">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="date_start">تاريخ البدء</label>
                                        <input type="text" name="date_start" id="date_start"
                                               class="form-control datetime"
                                               value="<?php echo til_get_date($task->date_start, 'datetime'); ?>" <?php if ($task->sen_u_id != get_active_user('id')): ?><?php endif; ?> >
                                    </div> <!-- /.form-group -->
                                </div> <!-- /.col-md-6 -->
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="date_end">تاريخ الإنجاز</label>
                                        <input type="text" name="date_end" id="date_end" class="form-control datetime"
                                               value="<?php echo til_get_date($task->date_end, 'datetime'); ?>" <?php if ($task->sen_u_id != get_active_user('id')): ?><?php endif; ?> >
                                    </div> <!-- /.form-group -->
                                </div> <!-- /.col-md-6 -->
                            </div> <!-- /.row -->
                            ...
                            <?php if (get_active_user('id') == $task->sen_u_id): ?>
                                ...
                                <input type="hidden" name="uniquetime" id="uniquetime_detail"
                                       value="<?php uniquetime(); ?>">
                            ...
                            <div class="text-right">
                                <?php if ($task->type_status == '0'): ?>
                                    <a href="?id=<?php echo $task->id; ?>&type_status=1" class="btn btn-default"><i
                                                class="fa fa-check-square-o"></i> إيقاف المهمة</a>
                                <?php else: ?>
                                    <a href="?id=<?php echo $task->id; ?>&type_status=0" class="btn btn-default"><i
                                                class="fa fa-check-square-o"></i> افتح المهمة مرة أخرى</a>
                                <?php endif; ?>
                            </div> <!-- /.text-right -->
                        </div> <!-- /.panel-body -->
                    </div> <!-- /.panel -->
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <?php if (!empty($choice) and is_object($choice)): ?>
                            ...
                            <div class="progress">
                                <div class="progress-bar progress-bar-stripedd <?php echo $progressbar_style; ?>"
                                     role="progressbar" aria-valuenow="<?php echo $part_completed; ?>" aria-valuemin="0"
                                     aria-valuemax="100" style="width: <?php echo $part_completed; ?>%;">
                                    <span class="sr-only"><?php echo $part_completed; ?>% مكتمل</span>
                                    %<?php echo $part_completed; ?> مكتمل
                                </div> <!-- /.progess-bar -->
                            </div> <!-- /.progress -->
                            ...
                            <form name="form" id="form" action="?id=<?php echo $task->id; ?>" method="POST">
                                        ...
                                        <ul class="list-group-item-task">
                                            ...
                                            <div class="text-right">
                                                <?php if ($arr->is_it_done): ?>
                                                    ...
                                                    <small class="text-muted italic"><span
                                                                data-wenk="<?php echo substr($arr->date, 0, 16); ?>"><?php echo get_time_late($arr->date); ?> منذ</span> <?php user_info($arr->user_id, 'display_name'); ?>
                                                        اكتمل بواسطة.
                                                    </small>
                                                <?php endif; ?>

                                                <input type="checkbox" name="choice_<?php echo $id; ?>"
                                                       id="choice_<?php echo $id; ?>"
                                                       <?php if ($arr->is_it_done): ?>checked<?php endif; ?>
                                                       class="toogle" data-size="mini" data-on-text="Evet"
                                                       data-off-text="Hayır">
                                            </div><!--/ .text-right /-->
                                        </ul><!--/ .list-group-item /-->
                                    <?php endforeach; ?>
                                ...
                                <div class="text-right">
                                    <button class="btn btn-success btn-lg span4"><i class="fa fa-save"></i> حفظ</button>
                                </div>
                            </form>

                        <?php else: ?>
                            <?php echo get_alert('لم يتم العثور على خيارات لهذه المهمة. <br /> يرجى قراءة نص الرسالة لاستكمال المهمة.', 'info', false); ?>
                        <?php endif; ?>
                    </div> <!-- /.panel-body /-->
                </div> <!-- /.panel /-->
            </div> <!-- /.col-md-8 /-->
            ...
            <div class="panel-body" style="paddig-bottom: 0;">
                <div class="row">
                    <div class="col-md-12">
                                                ...
                                                <?php if (get_active_user('id') != $message->sen_u_id): ?>
                                                    <div class="message-elem-avatar pull-left">
                                                        <img src="<?php echo get_user_info($message->sen_u_id, 'avatar'); ?>"
                                                             class="img-responsive br-3 pull-right" width="48">
                                                ...
                                                <div class="message-elem-content">
                                                    <div class="well padding-10 br-3">
                                                        <div class="text-muted fs-11 italic">
                                                            ...
                                                            <span class="inform-text">بواسطة</span> <span
                                                                    class="bold date-tooltip" <?php if (til_is_mobile()) {
                                                                echo 'data-wenk-pos="left"';
                                                            } ?> data-wenk="<?php echo substr($message->date, 0, 16); ?>"
                                                                    title="<?php echo substr($message->date, 0, 16); ?>"><?php echo get_time_late($message->date); ?></span>
                                                            <span class="inform-text">أرسلت قبل.</span>
                                                        </div><!--/ .text-muted /-->
                        ...
                        <!--/ TASK REPLY /-->
                        <form name="form_message" id="form_message" onsubmit="return send_message(this, 'task')"
                              js-onload="window.set_writing.top_id = '<?php echo $_GET['id']; ?>';" action=""
                                ...
                                <div class="col-md-1 hidden-xs">
                                    ...
                                    <?php if (get_active_user('avatar')): ?>
                                        <img src="<?php echo get_active_user('avatar'); ?>"
                                             class="img-responsive br-3 pull-right" width="64">
                                    <?php else: ?>
                                ...
                                <div class="col-md-11 col-xs-12">
                                    <?php if (til_is_mobile()) : ?>
                                        <div class="form-group message-area chat-input">
                                            <div class="user-input-control"
                                                 js-onload="get_writing('.user-input-control')">
                                            ...
                                            <input autofocus type="text"
                                                   onkeypress="if ( this.value.length > 0 ) { set_writing({'set_value': '1'}); } if ( event.keyCode == 13 ) { set_writing({'set_value': '0'}); } if ( event.keyCode == 8 ) { set_writing({'set_value': '2'}); } if ( this.value.length == 0 ) { set_writing({'set_value': '0'}); }"
                                                   onfocusout="set_writing({'set_value': '0'})"
                                                   onfocus="set_writing({'set_value': '1'})" name="message" id="message"
                                                   required class="form-control send-message-input" value=""
                                                   placeholder="Birşeyler yazın...">
                                            <button type="button" class="send-message-image"
                                                    onclick="document.getElementById('send-message-file').click()"><i
                                                        class="fa fa-image"></i></button>
                                            <button type="submit" class="send-message-submit"
                                                    onclick="document.getElementById('message').focus(); set_writing({'set_value': '1'});">
                                                <i class="fa fa-send"></i></button>
                                            <input type="file" name="" id="send-message-file"
                                                   onchange="var chat_list = document.querySelector('.chat-container'); chat_list.classList.add('loader'); imageHandler(this.files[0], function(data) { if ( data == false ) { chat_list.classList.remove('loader'); } else { if ( document.getElementById('message').value = '<img src='+ data +' class=img-responsive>' ) { chat_list.classList.remove('loader'); document.querySelector('.send-message-submit').click(); } } });"
                                                   value="" class="hidden">
                                    ...
                                    <?php else: ?>
                                        <div class="form-group message-area">
                                            ...
                                            <label for="message"
                                                   class="text-muted"><?php echo _b($rec_user->name . ' ' . $rec_user->surname); ?>
                                                اكتب رسالة لإرسالها...</label>
                                            <textarea autofocus onkeypress="set_writing({'set_value': '1'});"
                                                      onfocusout="set_writing({'set_value': '0'})"
                                                      onfocus="set_writing({'set_value': '1'})"
                                                      onkeydown="set_writing({'set_value': '0'}); parent(this, 'form').dispatchEvent(new Event('submit', { 'bubbles' : true, 'cancelable' : true}));"
                                                      name="message" id="message" class="form-control required"
                                                      minlength="5" placeholder="Birşeyler yazın..."
                                                      style="height:20px;"></textarea>
                                            <script>editor({
                                                    selector: "#message",
                                                    plugins: 'autolink nonbreaking table textcolor colorpicker image textpattern link',
                                    ...
                                    <div class="pull-right">
                                        <button type="submit" class="btn btn-primary pull-right hidden-xs"><i
                                                    class="fa fa-send-o"></i> ارسال
                                        </button>
                                        <input type="hidden" name="uniquetime" value="<?php uniquetime(); ?>">
                                        <input type="hidden" name="reply_message">
                                        <input type="hidden" name="task_id" id="task_id"
                                               value="<?php echo @$_GET['id']; ?>">
                                        <input type="hidden" name="top_id" id="top_id"
                                               value="<?php echo @$_GET['id']; ?>">
                                    </div> <!-- /.pull-right /-->
                                </div> <!-- /.col-md-11 /-->
                            </div> <!-- /.row /-->
