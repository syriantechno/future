...
if(!$account = get_account($_GET['id'])) {
	add_console_log('لم يتم العثور على بطاقة حساب.', 'print-address.php');
	exit;
}
...
<title>طباعة الباركود البضائع | <?php echo $account->name;?></title>
<?php
$args['logo'] = false;
$args['footer'] = false;
?>
<?php get_header_print($args); ?>
	...
	<div class="row space-none">
		<div class="col-xs-2 pr-1">
			<div class="fs-18 text-muted rotate90" style="margin-top: 80px;">العائدات</div>
		</div>
	...
	<div class="row space-none">
		...
		<div class="col-xs-2 pr-1">
			<div class="fs-18 text-muted rotate90" style="margin-top: 90px;">الواردات</div>
		</div>
		<div class="col-xs-10 pl-1">
