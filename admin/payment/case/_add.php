...
if(isset($_POST['add'])) {
	...
	@form_validation($_POST['name'], 'status_name', 'اسم صندوق النقد', 'required|min_length[3]|max_length[64]', 'input_control_');
	...
	<div class="col-lg-6 col-md-6">
		<form name="form_status" id="form_status" action="<?php set_url_parameters(array('add'=>array('taxonomy'=>$_taxonomy) )); ?>" method="POST" class="validate">
			...
			<div class="form-group">
				<label for="name">الصندوق او اسم البنك</label>
				...
				<?php if($cases): ?>
					<table class="table table-hover table-condensed">
						...
						<?php foreach($cases as $case):  $case->total = calc_case($case->id); ?>
							<tr>
								<td><a href="?detail&status_id=<?php echo $case->id; ?>"><span><?php echo $case->name; ?></span></a> <?php if($case->is_default == 'default'): ?><small class="text-muted">(افتراضي)</small><?php endif; ?></td>
				...
				<?php if($banks): ?>
					<table class="table table-hover table-condensed">
						...
						<?php foreach($banks as $bank): $bank->total = calc_case($bank->id); ?>
							<tr>
								<td><a href="?detail&status_id=<?php echo $bank->id; ?>"><span><?php echo $bank->name; ?></span></a> <?php if($bank->is_default == 'default'): ?><small class="text-muted">(افتراضي)</small><?php endif; ?></td>
