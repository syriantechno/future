	...
	<div class="row">
		<div class="col-md-4">
			<form name="form_status" id="form_status" action="" method="POST" class="validate">
				...
				<div class="form-group">
					<label class="veritical-center"><input type="checkbox" name="default" id="default" value="true" class="toogle" <?php checked($case->is_default, 'default'); ?> data-size="mini" data-on-text="Evet" data-off-text="Hayır"> &nbsp; هو الافتراضي المحدد، أي عندما يتم إنشاء النموذج الأول?</label>
				</div> <!-- /.form-group -->
			...
			<br />
			<label>هل تريد حذف المخزن? <a href="#" class="text-danger" data-toggle="modal" data-target="#form_status_delete"><i class="fa fa-trash"></i> Evet, kasayı sil</a></label>
			  ...
			  <form name="status_delete" id="status_delete" action="" method="GET" class="modal-dialog modal-md" role="document">
			    <div class="modal-content panel-danger">
			      ...
			      <div class="modal-body">
			        ...
			        <div class="alert <?php echo ($case->count > 0)? 'alert-danger' : 'alert-warning'; ?>"><?php echo $case->name; ?> آمنة, <?php echo _b($case->count); ?> العدد ödeme formu bulundu.</div>
			        ...
			        <?php if($case->count > 0): ?>
			        	<?php if($form_status_all): ?>
				        	<p><small class="text-muted">يرجى تحديد حالة نموذج مختلف من القائمة أدناه. <b><?php echo $case->name; ?></b> حالة النموذج <?php echo _b($case->count); ?> فإن شكل جهاز كمبيوتر شخصى تتغير مع وضع النموذج الذي حدد أدناه.</small></p>
				        	...
				        	<div class="h-10"></div>
				        	<p class="text-danger"><b><?php echo $case->name; ?></b> لحذف حالة النموذج و <?php echo _b($case->count); ?> هل تريد نقل النموذج إلى شكل النموذج الجديد أعلاه?</p>
					    <?php else: ?>
				        		<p><small class="text-muted"><b><?php echo $case->name; ?></b> لا يمكنك حذف حالة النموذج. <br /> لأنه في شكل <?php echo _b($case->count); ?> لا يمكن العثور على حالات شكل آخر لنقل النموذج. </small></p>
						<?php endif; ?>
					<?php else: ?>
						<?php if($form_status_all): ?>
							<p><small class="text-muted"><b><?php echo $case->name; ?></b> هل تريد حذف حالة النموذج؟ </small></p>
						<?php else: ?>
			      ...
			      <div class="modal-footer">
			        ...
			        <?php if($form_status_all): ?><button type="submit" class="btn btn-primary">نعم، أوافق</button><?php endif; ?>
			      </div> <!-- /.modal-footer -->
			    </div> <!-- /.modal-content -->
			  </form>
			</div> <!-- /.modal -->
