						...
						if ( $q_select->num_rows ) {
							...
							if ( $db->query("INSERT INTO {$_database['prefix']}users (username, password, email, name, surname, gsm, gender, role) VALUES ('". $_user['username'] ."', '". $_user['password'] ."', '". $_user['email'] ."', '". $_user['name'] ."', '". $_user['surname'] ."', '". $_user['gsm'] ."', '1', '1')") ) {
								if ( $db->insert_id ) {
									...
									$_POST['step_3'] = true;
								} else { add_alert('Kullan?c? Hesab? Eklenemedi.'); }
							} else{ add_alert('خطا الارتباط بقاعدة البيانات.'); }
						}
					} else { add_alert('MYSQL ERROR?!'); }
				} else { add_alert('هناك طريقة للذهاب. هل تريد الإعداد مرة أخرى؟ يمكنك محاولة.'); }
			}
...
<body>
	<div class="container">
		...
		<div class="form-container">
			...
			<?php print_alert(); ?>
			<small class="text-center block">الحصول على الدعم <a href="http://ultra.org/forum" class="bold">"ultra"</a> forumlar?n? ziyaret ediniz.</small>
			<?php exit; ?>
		</div>
		<?php endif; ?>
					...
					<?php if ( !isset($_POST['step_1']) AND !isset($_POST['step_2']) ): ?>
						<div class="setup-content">
							...
							<div class="form-group">
								<label for="host">اسم المخدم <small class="text-muted italic ml-10">(  الافتراضي"localhost" )</small></label>
							...
							<div class="form-group">
								<label for="table_prefix">لاحقة الجداول<small class="text-muted italic ml-10">( المفضلة "_erp" )</small></label>
								<input type="text" name="table_prefix" id="table_prefix" dynamic-input class="form-control" placeohlder="" value="erp_">
							</div><!--/ .form-group /-->
								...
								<div class="row space-5">
									...
									<div class="col-md-6">
										<div class="form-group">
											...
											<input type="text" name="user_gsm" id="user_gsm"  maxlength="20"  dynamic-input required class="form-control number" placeohlder="">
										</div><!--/ .form-group /-->
									</div><!--/ .col-md-6 /-->
								</div><!--/ .row.space-5 /-->
